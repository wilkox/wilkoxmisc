...
theme_wilkox <- function() {
  ...
  DIN <- "DIN" %in% names(pdfFonts()) & "DINCond-Medium" %in% names(pdfFonts())
  RegularFont <- ifelse(DIN, "DIN", "Helvetica")
  CondensedFont <- ifelse(DIN, "DINCond-Medium", "Helvetica-Narrow")
  ...
  theme_bw(base_size = 8, base_family = RegularFont) %+replace% theme(
    ...
    axis.title.x = element_text(vjust = 0, size = 10, family = RegularFont),
    axis.title.y = element_text(vjust = 1, angle = 90, size = 10, family = RegularFont),
