...
#'
#' @param TaxonTable taxon table in tidy format, i.e. with columns for
#' "Sample", "RelativeAbundance", and at least one rank. Use
#' "add.relative.abundance" first if the table only contains counts.
#' @param Rank the rank to collapse.
#' @param n the total number of groups following the collapse, including any
#' "Minor" and "Unclassified" groups.
#' @param MergeMinorUnclassified if set to FALSE, will keep minor and
#' unclassified taxa as separate (but still collapsed) groups.
...
collapse.taxon.table <- function (TaxonTable, Rank = "Phylum", n = 8, MergeMinorUnclassified = TRUE, UnclassifiedTerms = c(""), silent = TRUE) {
